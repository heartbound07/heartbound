...
public class DiscordPairingChannelService {
    ...
    /**
     * 🆕 NEW: Creates a Discord text channel for a group of 10 users
     * 
     * @param groupDiscordIds List of Discord IDs for group members
     * @param pairingId The pairing/group ID for reference
     * @return CompletableFuture containing channel creation result
     */
    public CompletableFuture<ChannelCreationResult> createGroupChannel(List<String> groupDiscordIds, Long pairingId) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                logger.info("Creating group channel for {} users (pairing ID: {})", 
                           groupDiscordIds.size(), pairingId);
                
                // Input validation
                if (groupDiscordIds == null || groupDiscordIds.isEmpty() || pairingId == null) {
                    throw new IllegalArgumentException("Group Discord IDs and pairing ID cannot be null or empty");
                }
                
                if (groupDiscordIds.size() != 10) {
                    throw new IllegalArgumentException("Group must have exactly 10 users, got " + groupDiscordIds.size());
                }
                
                // Validate all Discord IDs
                for (String discordId : groupDiscordIds) {
                    if (!isValidDiscordId(discordId)) {
                        throw new IllegalArgumentException("Invalid Discord ID format: " + discordId);
                    }
                }
                ...
                // Validate that all users are members of the server
                List<Member> groupMembers = new java.util.ArrayList<>();
                for (String discordId : groupDiscordIds) {
                    Member member = validateServerMember(guild, discordId);
                    if (member == null) {
                        return ChannelCreationResult.failure("User " + discordId + " is not a member of the Discord server");
                    }
                    groupMembers.add(member);
                }
                
                // Generate group channel name
                String channelName = generateGroupChannelName(pairingId);
                
                // Get category (optional, can be null)
                Category category = getPairingCategory();
                
                // Create the text channel with permissions for all group members
                TextChannel channel = createGroupTextChannelWithPermissions(guild, channelName, category, groupMembers);
                
                logger.info("Successfully created group channel '{}' (ID: {}) for {} users", 
                           channelName, channel.getId(), groupMembers.size());
                
                // Send welcome message to the group channel
                sendGroupWelcomeMessage(channel, groupMembers, pairingId);
                
                return ChannelCreationResult.success(channel.getId(), channelName);
                
            } catch (Exception e) {
                logger.error("Failed to create group channel for {} users: {}", 
                           groupDiscordIds.size(), e.getMessage(), e);
                return ChannelCreationResult.failure("Group channel creation failed: " + e.getMessage());
            }
        }).orTimeout(60, TimeUnit.SECONDS) // Longer timeout for group operations
          .exceptionally(throwable -> {
              logger.error("Timeout or error in group channel creation: {}", throwable.getMessage());
              return ChannelCreationResult.failure("Group channel creation timed out");
          });
    }

    /**
     * 🆕 NEW: Generate a name for a group channel
     */
    private String generateGroupChannelName(Long pairingId) {
        String baseName = "group-" + pairingId;
        return sanitizeForChannelName(baseName);
    }

    /**
     * 🆕 NEW: Create a text channel with permissions for all group members
     */
    private TextChannel createGroupTextChannelWithPermissions(Guild guild, String channelName, Category category, 
        ...
        try {
            ...
            channelAction = channelAction.addRolePermissionOverride(
                guild.getPublicRole().getIdLong(),
                null, // no allow permissions
                EnumSet.of(Permission.VIEW_CHANNEL) // deny view
            );
            
            // Grant permissions to all group members
            for (Member member : groupMembers) {
                channelAction = channelAction.addMemberPermissionOverride(
                    member.getIdLong(),
                    EnumSet.of(
                    ...
                    null // no deny permissions
                );
            }
            
            TextChannel channel = channelAction.complete();
            logger.info("Successfully created group text channel '{}' with permissions for {} members", 
                       channelName, groupMembers.size());
            
            return channel;
            
        } catch (InsufficientPermissionException e) {
            logger.error("Insufficient permissions to create group channel '{}': {}", channelName, e.getMessage());
            throw new RuntimeException("Bot lacks permission to create group channel: " + e.getPermission());
        } catch (Exception e) {
            logger.error("Failed to create group channel '{}': {}", channelName, e.getMessage());
            throw new RuntimeException("Group channel creation failed: " + e.getMessage());
        }
    }

    /**
     * 🆕 NEW: Send welcome message to a newly created group channel
     */
    private void sendGroupWelcomeMessage(TextChannel channel, List<Member> groupMembers, Long pairingId) {
        try {
            // Create mention string for all group members
            StringBuilder mentions = new StringBuilder("Hey ");
            for (int i = 0; i < groupMembers.size(); i++) {
                mentions.append(groupMembers.get(i).getAsMention());
                if (i < groupMembers.size() - 2) {
                    mentions.append(", ");
                } else if (i == groupMembers.size() - 2) {
                    mentions.append(" and ");
                }
            }
            mentions.append("! ");
            
            // Create the embed with group information
            EmbedBuilder embed = buildGroupWelcomeMessageEmbed(groupMembers, pairingId);
            
            // Send the welcome header message first
            channel.sendMessage(mentions.toString()).queue(
                success -> {
                    // Then send the embed
                    channel.sendMessageEmbeds(embed.build()).queue(
                        embedSuccess -> logger.info("Successfully sent group welcome message and embed to channel: {}", channel.getName()),
                        embedError -> logger.warn("Failed to send group welcome embed to channel {}: {}", channel.getName(), embedError.getMessage())
                    );
                },
                error -> logger.warn("Failed to send group welcome message to channel {}: {}", channel.getName(), error.getMessage())
            );
            
        } catch (Exception e) {
            logger.error("Error sending group welcome message to channel {}: {}", channel.getName(), e.getMessage());
        }
    }

    /**
     * 🆕 NEW: Build the group welcome message embed
     */
    private EmbedBuilder buildGroupWelcomeMessageEmbed(List<Member> groupMembers, Long pairingId) {
        EmbedBuilder embed = new EmbedBuilder();
        
        embed.setColor(Color.decode("#ff6b9d"));
        embed.setTitle("🎉 Welcome to your Group Channel! 🎉");
        
        // About this channel section
        StringBuilder aboutChannel = new StringBuilder();
        aboutChannel.append("**👥 About this group:**\n");
        aboutChannel.append("• This is your **private group channel** - only you ").append(groupMembers.size()).append(" can see it\n");
        aboutChannel.append("• Perfect place for everyone to chat, get to know each other, and have fun!\n");
        aboutChannel.append("• Feel free to share memes, talk about games, or organize activities together\n");
        
        // What happens next section
        StringBuilder nextSteps = new StringBuilder();
        nextSteps.append("**💫 What happens next:**\n");
        nextSteps.append("• Get to know each other in this group setting\n");
        nextSteps.append("• If you want to pair up with someone specific, you can request it later\n");
        nextSteps.append("• Individual pairing features will be available through slash commands\n");
        nextSteps.append("• For now, just enjoy meeting new people!\n");
        
        // Group info section
        StringBuilder groupInfo = new StringBuilder();
        groupInfo.append("**🔍 Group Information:**\n");
        long maleCount = groupMembers.stream().filter(m -> true).count() / 2; // Assuming equal split for now
        long femaleCount = groupMembers.size() - maleCount;
        groupInfo.append("• **Total members:** ").append(groupMembers.size()).append("\n");
        groupInfo.append("• **Group composition:** Mixed gender group\n");
        groupInfo.append("• **Same region:** All members are from the same region for better compatibility\n");
        
        // Rules section
        StringBuilder rules = new StringBuilder();
        rules.append("**📋 Channel Guidelines:**\n");
        rules.append("• Be respectful and kind to everyone\n");
        rules.append("• Keep conversations appropriate and fun\n");
        rules.append("• No harassment or inappropriate behavior\n");
        rules.append("• Have fun and be yourself!\n");
        
        // Add fields to embed
        embed.addField("", aboutChannel.toString(), false);
        embed.addField("", nextSteps.toString(), false);
        embed.addField("", groupInfo.toString(), false);
        embed.addField("", rules.toString(), false);
        
        // Footer message
        embed.setFooter("Group ID: " + pairingId + " • Heartbound Matchmaking", null);
